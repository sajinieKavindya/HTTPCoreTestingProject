...
public class ClientSendsInvalidChunks {
    ...
    public ClientSendsInvalidChunks(String host, int port) {
        this.host = host;
        this.port = port;
    }
    ...
    public void run(String payload, RequestMethods method) {
        try {
            ...
            new ClientSendsInvalidChunks.ClientThread(socket, payload, method).start();
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    ...
    static class ClientThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                InputStream stream = new ByteArrayInputStream(this.payload.getBytes(StandardCharsets.UTF_8));
